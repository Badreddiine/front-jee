...
import { Plus, Search, Users, X, CheckCircle2 } from "lucide-react"
...
export default function GroupsContent() {
  ...
  const handleCreateGroup = async (e: React.FormEvent) => {
    ...
    try {
      const newGroup = await groupeApi.create(formData)
                  ...
                  <Card className={`hover:shadow-lg transition-shadow h-full ${!isMember ? 'opacity-90' : 'cursor-pointer'}`}>
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        ...
                        <div className={`px-2 py-1 rounded text-xs font-semibold ${getTypeColor(group.type)}`}>
                          {group.type}
                        </div>
                      </div>
                    </CardHeader>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between text-sm pt-2 border-t border-border">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Created {new Date(group.dateCreation).toLocaleDateString()}
                          </div>
                        </div>
